...
public class ProductController {
    ...
    /**
     * Actualización parcial de un producto usando PATCH
     */
    @Operation(
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<com.eafit.tutorial.dto.ApiResponse<ProductDTO>> patchProduct(
            @Parameter(description = "ID del producto a actualizar", example = "1", required = true)
            @PathVariable @Min(1) Long id,
        ...
        logger.debug("PATCH /api/v1/products/{} - updating fields", id);

        try {
            if (!updateProductDTO.hasUpdates()) {
                return ResponseEntity.badRequest()
                    .body(com.eafit.tutorial.dto.ApiResponse.error(
                        "Debe proporcionar al menos un campo para actualizar", 400));
            }

            Product existingProduct = productService.getProductById(id)
                .orElseThrow(() -> new RuntimeException("Producto no encontrado con ID: " + id));

            // Aplicar solo los campos que no son null
            ...
            logger.error("Error al actualizar parcialmente producto con ID: {}", id, e);
     ...
     */
    @Operation(
        operationId = "getProductsWithLowStock",
    ...
    @Operation(
        ...
        description = "Obtiene estadísticas generales sobre los productos en el sistema",
        tags = {"Products - Statistics"}
    )
    @GetMapping("/stats")
    public ResponseEntity<com.eafit.tutorial.dto.ApiResponse<Object>> getProductStats() {
        logger.debug("GET /api/v1/products/stats");

        try {
            var stats = java.util.Map.of(
                "totalProducts", productService.getAllProducts().size(),
                "activeProducts", productService.getAllProducts().stream()
                    .mapToInt(p -> p.getActive() ? 1 : 0).sum(),
                "categoriesCount", productService.getAllProducts().stream()
                    .map(Product::getCategory).distinct().count(),
                "lowStockProducts", productService.getProductsWithLowStock(5).size()
            );

            return ResponseEntity.ok(
                com.eafit.tutorial.dto.ApiResponse.success(stats,
